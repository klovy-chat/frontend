...
import {
  ...
  LISTENING_SYNC_PROVIDER,
  syncListeningActivity,
} from "../api/integrations";
import { useAuth } from "../context/AuthContext";
import { getClientInstanceId } from "../utils/env/clientInstanceId";
import { LISTENING_CONNECTION_CHANGED } from "../utils/sync/listeningConnectionSync";
import type { User } from "../types";

const SYNC_INTERVAL_MS = 45_000;
const STATUS_STORAGE_KEY = "klovy.listening.connected";
...
export function useIntegrationListeningPoll(): void {
  ...
  useEffect(() => {
    ...
    const refreshStatus = async () => {
      try {
        const status = await getIntegrationStatus(LISTENING_SYNC_PROVIDER);
        if (cancelled) return;
        connectedRef.current = status.connected;
        shareRef.current = status.shareListening ?? true;
        ...
        if (current) {
          const connectedAccounts =
            status.connected && status.accountName && status.profileUrl
              ? [
                  {
                    provider: LISTENING_SYNC_PROVIDER,
          ...
          updateUser({
            ...current,
            shareListening: status.shareListening ?? current.shareListening,
            spotifyConnected: status.connected,
            connectedAccounts,
            listeningActivity: status.connected ? current.listeningActivity : null,
          });
        }
      } catch {
        if (!cancelled) {
    ...
    window.addEventListener(LISTENING_CONNECTION_CHANGED, onConnectionChanged);

    return () => {
      cancelled = true;
      window.removeEventListener(LISTENING_CONNECTION_CHANGED, onConnectionChanged);
    };
  ...
  useEffect(() => {
    ...
    const runSync = async () => {
      ...
      try {
        const result = await syncListeningActivity(LISTENING_SYNC_PROVIDER, {
          clientType: "web",
          clientInstanceId,
        });
