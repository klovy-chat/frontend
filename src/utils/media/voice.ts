// voice.ts
// Ustawienia mikrofonu w storage + opisy błędów getUserMedia.
// Zakres:
//  - deviceId constraints
//  - deviceId mikrofonu czatu/call + opisy błędów getUserMedia
// Call i VoiceSettings czytają to samo — nie duplikuj klucza localStorage.
// Przy zmianach: VoiceSettings.tsx, CallContext.tsx.

import i18n from "../../i18n/config";

import type { ScreenShareQualityId } from "../call/screenShare";
import { DEFAULT_SCREEN_SHARE_QUALITY, isScreenShareQualityId } from "../call/screenShare";
...
function audioDevicesHaveLabels(devices: MediaDeviceInfo[]): boolean {
  return devices.some(
    (device) =>
...
export async function listAudioDevices(): Promise<{
  ...
  return { inputs, outputs, hasLabels: audioDevicesHaveLabels(devices) };
}
