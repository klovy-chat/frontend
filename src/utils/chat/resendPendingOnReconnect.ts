...
import { wrapOutgoingContent } from "../../crypto/messageContent";
...
/** Shell-owned: scrub aged + resend young optimistics after WS reconnect. */
export function resendPendingOnReconnect(
  ...
  for (const { key, message: m } of young) {
    ...
    void (async () => {
      // Soft !sent after reconnect — keep pending for TTL scrub / later retry.
      // Hard drop only when send throws (protocol/session death).
      try {
        if (isDm) {
