...
function tipIsStaleAgainst(
  ...
  // Same-ms: prefer non-temp server id over optimistic temp-*.
  // tipIdNewerPreferNonTemp(existing, incoming) ⇒ existing wins ⇒ incoming is stale.
  return tipIdNewerPreferNonTemp(entry.lastMessageId, msg._id);
}

/** Same-ms / equal-time: live tip wins only if not sticky temp vs server id. */
...
/** Optimistic own send — Sidebar tip without waiting for RECEIVE echo. */
...
/** Failed/dropped optimistic send — restore tip from cache excluding temp id. */
...
/** Update sidebar preview text when the last message is edited (no reorder). */
...
/** Clear or backfill preview when the deleted message was the list tip. */
export function patchContactsOnMessageDeleted(
  ...
  // Exclude deleted id — Sidebar may run before Bridge removes the row from cache.
