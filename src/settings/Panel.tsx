// Panel.tsx
// UI wszystkich sekcji ustawień (profil, konto, sesje, głos, język, ostrzeżenia).
// Zakres:
//  - zapis przez api/auth
//  - odświeżenie usera po sukcesie
...
export function Panel({
  ...
  useEffect(() => {
    if (section !== "konto") setMobileKontoView("hub");
  }, [section]);
    ...
    <div className="settings-mobile-hub">
      ...
      <nav className="settings-mobile-menu" aria-label={t("settings.nav.account")}>
        <button
          ...
          onClick={() => setMobileKontoView("account")}
        >
          ...
          <span className="settings-mobile-menu__label">{t("settings.nav.myAccount")}</span>
  ...
  const settingsSections = (
    <div key={section}>
          ...
          {section === "konto" && (
            <>
              {isMobile ? (
                mobileKontoView === "hub" ? mobileHub : null
              ) : (
                <>
              ...
              {(!isMobile || mobileKontoView === "account") && (
              <>
        ...
        <div className="app-shell__settings-content as-content as-content--inline">
          ...
          {isMobile && section === "konto" && mobileKontoView === "account" && (
            <div className="settings-mobile-bar">
              ...
              <h1 className="settings-mobile-bar__title">{t("settings.nav.myAccount")}</h1>
            </div>
          )}
