// LinkEmbeds.tsx
// Embedy pod treścią: karta, iframe, invite.
// Zakres:
//  - hide raw URL gdy treść to sam link
//  - karta OG, iframe, invite pod treścią
// Nowy host iframe: allowlista na BE + embeds.ts.
// Przy zmianach: utils/chat/embeds.ts, controllers/messages.rs.

import { useEffect, useMemo, useState } from "react";
import { fetchLinkPreview } from "../../api/messages";
import { safeHttpsHref } from "../../utils/chat/format";
import {
  resolveMessageLinkEmbeds,
  type LinkPreviewCard,
  type ResolvedLinkEmbed,
} from "../../utils/chat/embeds";
import { InviteCard } from "./InviteCard";
import "../../styles/chat/embeds.css";
  ...
  return (
    <div className="message-link-embeds">
      {inviteLinks.map((link) => (
        <InviteCard key={link.inviteId} link={link} />
      ))}
      {iframes.map((embed) => (
